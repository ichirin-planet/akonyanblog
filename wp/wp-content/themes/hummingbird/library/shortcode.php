...
function kanrenFunc($atts) {
	...
	foreach ($post as $item){
		$im = wp_get_attachment_image_src(get_post_thumbnail_id($item->ID),'medium',false);
		$date = date('Y.m.d',strtotime(get_post($item->ID)->post_date));
		$update = date('Y.m.d',strtotime(get_post($item->ID)->post_modified));
		$echo .= '<div class="related_article cf"><a href="'.get_permalink($item->ID).'" class="cf"><figure class="thum"><img src="'.$im[0].'" /></figure><div class="meta inbox"><p class="ttl">'.$item->post_title.'</p><span class="date">'.$date.'</span></div></a></div>';
	}
...
function kanren2Func($atts) {
	...
	foreach ($post as $item){
		...
		$date = date('Y.m.d',strtotime(get_post($item->ID)->post_date));
		$update = date('Y.m.d',strtotime(get_post($item->ID)->post_modified));
		$echo .= '<div class="related_article labelnone cf"><a href="'.get_permalink($item->ID).'" class="cf"><figure class="thum"><img src="'.$im[0].'" /></figure><div class="meta inbox"><p class="ttl">'.$item->post_title.'</p><span class="date">'.$date.'</span></div></a></div>';
	}
